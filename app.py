import os, cv2, time, json, argparse, threading, asyncio, platform
...
# ---------------- CLI ----------------
def parse_args():
    ap = argparse.ArgumentParser("Multi-camera backend (YOLO + zones) for React UI")
...
def list_local_cams() -> List[dict]:
    ...
    if platform.system() == "Windows" and FilterGraph is not None:
        try:
            ...
            # If pygrabber found nothing, fall back to probing
            if devices:
                return devices
...
def beep():
    """
    Returns the custom beep audio file.
    Place 'beep.wav' in the same directory as this file,
    ...
    def run(self):
        ...
        while not self.stop_flag:
            ...
            for b in boxes:
                ...
                color = ALERT_COLOR if in_zone else SAFE_COLOR
                if in_zone: triggered = True
...
if __name__ == "__main__":
    ...
    if ARGS.draw_zones:
        if not ARGS.cam_id or ARGS.cam_id not in CAM_IDS:
            ...
            raise SystemExit(1)
        # For drawing we keep a simple assumption that the original index in --cams matches.
        # Adjust if you want name->index resolution here as well.
